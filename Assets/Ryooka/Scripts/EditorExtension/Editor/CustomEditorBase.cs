	...
	public class CustomEditorBase: UnityEditor.Editor {
		...
		private class ReorderableListProperty {
			...
			private void CreateList() {
				...
				this.List.drawHeaderCallback += rect => EditorGUI.LabelField(rect, this._property.displayName);
				this.List.onCanRemoveCallback += (list) => { return this.List.count > 0; };
				this.List.drawElementCallback += this.drawElement;
				this.List.drawElementCallback += this.alwaysExpandElement;
			...
			private void alwaysExpandElement(Rect rect, int index, bool active, bool focused) {
				Property.GetArrayElementAtIndex(index).isExpanded = true;
            }
        }
		#endregion
	}
